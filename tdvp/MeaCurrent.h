...
MPO get_current_mpo2 (const SiteType& sites, const WireSystem& sys, int i)
{
    ...
    auto u1 = conj (sys.chain (p1).Ui(i1));
    auto u2 = sys.chain (p2).Ui(i2);
    ...
    for(int k1 = 1; k1 <= u1.size(); k1++)
        ...
        {
            int j1 = sys.orb_map (p1,k1);
            int j2 = sys.orb_map (p2,k2);
            if (j1 == j2)
            {
